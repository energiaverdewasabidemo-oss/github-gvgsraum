...
const OptimizedTestimonials = () => {
  ...
  return (
    <section 
      id="testimonios"
      ref={sectionRef}
      className="py-16 sm:py-20 lg:py-32 relative overflow-hidden bg-white min-h-screen flex items-center"
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div className={`text-center mb-16 sm:mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="inline-flex items-center space-x-3 bg-[#A8FF00]/20 px-8 py-4 rounded-full mb-8 border border-[#A8FF00]/50">
            <Quote className="h-6 w-6 text-[#1A1A1A]" />
            <span className="text-[#1A1A1A] font-bold text-xl">Testimonios</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10 mb-12 sm:mb-16">
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div className="bg-[#F5F5F0] rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 hover:border-[#A8FF00] relative overflow-hidden h-full group-hover:-translate-y-2 flex flex-col justify-between">
                ...
                <div className="flex-grow">
                  <p className="text-gray-600 leading-relaxed mb-6 sm:mb-8 italic text-base sm:text-lg">
                    "{testimonial.text}"
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <div>
                      <p className="font-bold text-[#1A1A1A] text-base sm:text-lg">{testimonial.name}</p>
                      <p className="text-gray-400 text-sm sm:text-base">{testimonial.location}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[#6BBF00] font-bold text-lg sm:text-xl">-{testimonial.savings}</p>
                    <p className="text-gray-400 text-xs sm:text-sm">ahorro anual</p>
                  </div>
                </div>
